...
/**
 * Tests multiple deploy/undeploy of MP FT deployments (just JAR archives)
 */
...
public class MultipleJarDeploymentTest {
    ...
    private static final String FIRST_DEPLOYMENT_JAR = "first-deployment";
     ...
     * @tpPassCrit Verify that second deployment does NOT change MP FT/Hystrix configuration.
    ...
    public void testFaultToleranceTimeoutOnSecondDeployment() throws Exception {
        MatcherAssert.assertThat("Microprofile Fault Tolerance does not work on 2nd deployment.",
                helloService.timeout(true), CoreMatchers.containsString("Fallback Hello, context = foobar"));
    }
}
