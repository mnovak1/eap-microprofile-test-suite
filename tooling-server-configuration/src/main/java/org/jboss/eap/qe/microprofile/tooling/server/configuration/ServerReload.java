...
public class ServerReload {
     ...
     * @param serverAddress if {@code null}, use {@code TestSuiteEnvironment.getServerAddress()} to create the
     ...
     */
    public static void executeReloadAndWaitForCompletion(ModelControllerClient client, int timeout, boolean adminOnly,
            String serverAddress, int serverPort, String serverConfig) {
        executeReload(client, adminOnly, serverConfig);
        ArquillianContainerProperties arquillianContainerProperties = new ArquillianContainerProperties(
                ArquillianDescriptorWrapper.getArquillianDescriptor());
        try {
            waitForLiveServerToReload(timeout,
                    serverAddress != null ? serverAddress : arquillianContainerProperties.getDefaultManagementAddress(),
                    serverPort != -1 ? serverPort : arquillianContainerProperties.getDefaultManagementPort());
    ...
    /**
     * Check if the server is in reload-required state and if it is, execute
     * {@link #executeReloadAndWaitForCompletion(ManagementClient)}.
     *
     * @param managementClient client to use to execute the reload and to provide host and port information for
     *        * reconnecting to wait for completion of the reload
